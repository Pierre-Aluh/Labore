...
function LoginScreen({ onSubmit, error }: { onSubmit: (event: FormEvent<HTMLFormElement>) => void; error: string }) { return <main className="reference-login"><div className="reference-login-brand"><img src="/assets/labore-logo.png" alt="Labore" /></div><h1>Acesso restrito</h1><form className="reference-login-card" onSubmit={onSubmit}><div className="reference-login-fields"><label>Login<input name="email" type="email" autoComplete="username" required /></label><label>Senha<input name="password" type="password" autoComplete="current-password" required /></label></div>{error && <div className="error">{error}</div>}<button className="reference-submit" type="submit">Entrar</button></form><span className="reference-version">Ver. Teste</span></main>; }
